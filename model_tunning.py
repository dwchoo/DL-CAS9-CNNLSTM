...
class CNNLSTM_short_HP_tunning:
    def __init__(self, default_HP_dict, log_dir='tensorboard_log/'):
        self.log_dir = log_dir
    ...
    def tunning_model(self, HP_list, GPU = 1, random_seed =1234 ,log_dir='tensorboard_log/',
        ...
        try:
            ...
            self.model_tensorboard = CNNLSTM_short_tensorboard(
                HP_dict=HP_list2dict,
                log_dir=log_dir,
                GPU=GPU,
                random_seed=random_seed,
    ...
    def change_HP_scale(self, HP_list, changed_scale = False, show_items = False):
        if changed_scale is False:
            HP_list2dict = {
                    ...
                    'DNN_1'         : int(HP_list[4]),
                    'DNN_2'         : int(HP_list[5]),
                    'DNN_rate'      : int(HP_list[6]),

                    'CNN_dropout'   : float(HP_list[7]),
                    'DNN_dropout'   : float(HP_list[8]),

                    'learning_rate' : float(HP_list[9])
                }
        else:
            HP_list2dict = {
                'CNN_1_kernal'  : int(2**HP_list[0]) ,
                ...
                'DNN_1'         : int(2**HP_list[4]),
                'DNN_2'         : int(2**HP_list[5]),
                'DNN_rate'      : int(2**HP_list[6]),
...
class SpCas9_HP_tunning:
    def __init__(self,
        ...
        self.__default_HP = default_HP_dict
        self.log_dir = log_dir
        self.__multi_case_tunning = multi_case_tunning
    ...
    def HP_dimension_define(self,):
        ...
        self.__change_HP_scale_bool = True        
        self.__CNN_1_kernal   = Integer(low=0, high=10, name= 'CNN_1_kernal_node')
        self.__CNN_1_RNN      = Integer(low=0, high=10, name= 'CNN_1_RNN_node')
        self.__CNN_2_kernal   = Integer(low=0, high=10, name= 'CNN_2_kernal_node')
        self.__CNN_2_RNN      = Integer(low=0, high=10, name= 'CNN_2_RNN_node')
        self.__CNN_3_kernal   = Integer(low=0, high=10, name= 'CNN_3_kernal_node')
        self.__CNN_3_RNN      = Integer(low=0, high=10, name= 'CNN_3_RNN_node')
        self.__CNN_4_kernal   = Integer(low=0, high=10, name= 'CNN_4_kernal_node')
        self.__CNN_4_RNN      = Integer(low=0, high=10, name= 'CNN_4_RNN_node')
        self.__CNN_5_kernal   = Integer(low=0, high=10, name= 'CNN_5_kernal_node')
        self.__CNN_5_RNN      = Integer(low=0, high=10, name= 'CNN_5_RNN_node')
        self.__DNN_1          = Integer(low=0, high=10, name= 'DNN_1_node')
        self.__DNN_2          = Integer(low=0, high=10, name= 'DNN_2_node')
        self.__DNN_rate       = Integer(low=0, high=10, name= 'DNN_rate_node')
        self.__CNN_dropout    = Real(low=0.001, high= 0.5, name= 'CNN_dropout_node')
        self.__DNN_dropout    = Real(low=0.001, high= 0.5, name= 'DNN_dropout_node')
        self.__learning_rate  = Real(low=1e-6, high=0.1, prior='log-uniform', name='Learning_rate_node')
        ...
        self.__default_HP_list = list(self.__default_HP.values())
    ...
    def tunning_start(self, n_random_start=50, n_cell=100, x0=None,
        ...
        self.__now_cell = 1
        self.__totoal_cell = n_cell

        if self.__multi_case_tunning is False:
            self.gp_fitting = gp_minimize(
                func=self.tunning_func,
                dimensions=self.dimension_HP,
                n_calls=n_cell,
    ...
    def multiprocessing_process(self, HP_list, case_num=3, log_dir='tensorboard_log/', GPU_list = [0,1,2]):
        ...
        result_dict = {
            'test_rate_loss': result_test_rate_loss_list ,
            'test_pearson'  : result_test_pearson_list   ,
            'test_spearman' : result_test_spearman_list  ,
            'bench_pearson' : result_bench_pearson_list  ,
            'bench_spearman': result_bench_spearman_list ,
    ...
    def tunning_func_multiprocs(self, HP_list):
        print("====================== start trainning =======================")
        print("progress : {}/{}".format(self.__now_cell, self.__totoal_cell))
        
        #print(HP_list)

        HP_list2dict = self.change_HP_scale(
                HP_list=HP_list,
                changed_scale=self.__change_HP_scale_bool,
                show_items=True
                )
        ...
        try:
            multiprocessing_result = self.multiprocessing_process(
                HP_list= HP_list,
                case_num= self.__case_num,
                log_dir= self.__log_dir,
                GPU_list= self.__GPU_list
            )
    ...
    def tunning_model(self, HP_list, GPU = 1, random_seed =1234 ,log_dir='tensorboard_log/',
        ...
        try:
            ...
            self.model, self.result_dict = self.model_define(
            ...
            self.display_model_result(
                test_loss       = self.result_dict['test_rate_loss' ],
                test_pearson    = self.result_dict['test_pearson'   ],
                test_spearman   = self.result_dict['test_spearman'  ],
                bench_pearson   = self.result_dict['bench_pearson'  ],
                bench_spearman  = self.result_dict['bench_spearman' ],
            )

            if proc_num is not None and return_dict is not None:
                return_dict[proc_num] = self.result_dict

            test_rate_loss = self.result_dict['test_rate_loss']
    ...
    def model_define(self, HP_list2dict, log_dir, GPU, random_seed, verobse):
        ...
        ###
        #_tensorboard_model._model_define.epochs = 3
        ...
        test_loss       = _tensorboard_model.evaluate_test_loss
        test_pearson    = _tensorboard_model.evaluate_test_pearson
        test_spearman   = _tensorboard_model.evaluate_test_spearman
        bench_pearson   = _tensorboard_model.evaluate_bench_pearson
        bench_spearman  = _tensorboard_model.evaluate_bench_spearman
