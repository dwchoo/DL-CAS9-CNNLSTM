...
class SpCas9_tensorboard:
    ...
    def TB_HParam_define(self):
        ...
        # HP
        self._CNN_1_kernal = hp.HParam('CNN_1_kernal',   hp.IntInterval(1, 1024))
        self._CNN_1_RNN    = hp.HParam('CNN_1_RNN',      hp.IntInterval(1, 1024))
        self._CNN_2_kernal = hp.HParam('CNN_2_kernal',   hp.IntInterval(1, 1024))
        self._CNN_2_RNN    = hp.HParam('CNN_2_RNN',      hp.IntInterval(1, 1024))
        self._CNN_3_kernal = hp.HParam('CNN_3_kernal',   hp.IntInterval(1, 1024))
        self._CNN_3_RNN    = hp.HParam('CNN_3_RNN',      hp.IntInterval(1, 1024))
        self._CNN_4_kernal = hp.HParam('CNN_4_kernal',   hp.IntInterval(1, 1024))
        self._CNN_4_RNN    = hp.HParam('CNN_4_RNN',      hp.IntInterval(1, 1024))
        self._CNN_5_kernal = hp.HParam('CNN_5_kernal',   hp.IntInterval(1, 1024))
        self._CNN_5_RNN    = hp.HParam('CNN_5_RNN',      hp.IntInterval(1, 1024))
        self._DNN_1        = hp.HParam('DNN_1',          hp.IntInterval(1, 1024))
        self._DNN_2        = hp.HParam('DNN_2',          hp.IntInterval(1, 1024))
        self._DNN_rate     = hp.HParam('DNN_rate',       hp.IntInterval(1, 1024))
        self._CNN_dropout  = hp.HParam('CNN_dropout',    hp.RealInterval(0.001, 0.5))
        self._DNN_dropout  = hp.HParam('DNN_dropout',    hp.RealInterval(0.001, 0.5))
        self._learning_rate  = hp.HParam('learning_rate',hp.RealInterval(1e-6, 1.0))
        ...
        self._Metric_name_loss = 'rate_loss_test'
        self._Metric_name_test_pearson = 'pearson_correlation_test'
        self._Metric_name_test_spearman = 'spearman_correlation_test'
        self._Metric_name_bench_pearson = 'pearson_correlation_bench'
        self._Metric_name_bench_spearman = 'spearman_correlation_bench'

        with tf.summary.create_file_writer(self.log_dir + '/hp_tunning').as_default():
            hp.hparams_config(
                hparams=[self._CNN_1_kernal, self._CNN_1_RNN,
                         self._CNN_2_kernal, self._CNN_2_RNN,
                         self._CNN_3_kernal, self._CNN_3_RNN,
                         self._CNN_4_kernal, self._CNN_4_RNN,
                         self._CNN_5_kernal, self._CNN_5_RNN,
                         self._DNN_1, self._DNN_2, self._DNN_rate,
                         self._CNN_dropout, self._DNN_dropout,
                         self._learning_rate],
                metrics=[hp.Metric(self._Metric_name_loss, display_name='Loss'),
                         hp.Metric(self._Metric_name_test_pearson, display_name='Pearson Correlation - Test data'),
                         hp.Metric(self._Metric_name_test_spearman, display_name='Spearman Correlation - Test data'),
                         hp.Metric(self._Metric_name_bench_pearson, display_name='Pearson Correlation - Bench data'),
                         hp.Metric(self._Metric_name_bench_spearman, display_name='Spearman Correlation - Bench data')]
            )
    ...
    def callbacks(self):
        ...
        callbacks_plot = [test_data_correlation_plot, bench_data_correlation_plot]
        self._callbacks_list = self.__callbacks.TB_callbacks(plot_callbacks = callbacks_plot)
    ...
    def evaluate(self,):
        self.model_evaluation = self._model_define.model_evaluate(verbose=0)
        
        #print(model_evaluate)
        self._H_param(
            loss            = self.model_evaluation.test_rate_loss,
            pearson         = self.model_evaluation.test_pearson,
            spearman        = self.model_evaluation.test_spearman,
            bench_pearson   = self.model_evaluation.bench_pearson,
            bench_spearman  = self.model_evaluation.bench_spearman,
            step=1
        )
        pass
    ...
    def _H_param(self, loss, pearson, spearman, bench_pearson=None, bench_spearman=None,step=3):
        hparams = {
            'CNN_1_kernal'  : self._HP_dict['CNN_1_kernal'] ,
            'CNN_1_RNN'     : self._HP_dict['CNN_1_RNN']    ,
            'CNN_2_kernal'  : self._HP_dict['CNN_2_kernal'] ,    
            'CNN_2_RNN'     : self._HP_dict['CNN_2_RNN']    ,
            'CNN_3_kernal'  : self._HP_dict['CNN_3_kernal'] ,    
            'CNN_3_RNN'     : self._HP_dict['CNN_3_RNN']    ,
            'CNN_4_kernal'  : self._HP_dict['CNN_4_kernal'] ,    
            'CNN_4_RNN'     : self._HP_dict['CNN_4_RNN']    ,
            'CNN_5_kernal'  : self._HP_dict['CNN_5_kernal'] ,    
            'CNN_5_RNN'     : self._HP_dict['CNN_5_RNN']    ,
            'DNN_1'         : self._HP_dict['DNN_1']        ,
            'DNN_2'         : self._HP_dict['DNN_2']        ,
            'DNN_rate'      : self._HP_dict['DNN_rate']     ,
            'CNN_dropout'   : self._HP_dict['CNN_dropout']  ,    
            'DNN_dropout'   : self._HP_dict['DNN_dropout']  ,
            'learning_rate' : self._HP_dict['learning_rate'],
        }
        
        with tf.summary.create_file_writer(self.log_dir + '/hp_tunning').as_default():
            hp.hparams(hparams=hparams)
            #print(loss, pearson, spearman)
            tf.summary.scalar(self._Metric_name_loss, loss, step=step)
            tf.summary.scalar(self._Metric_name_test_pearson, pearson, step=step)
            tf.summary.scalar(self._Metric_name_test_spearman, spearman, step=step)
            if bench_pearson is not None:
                tf.summary.scalar(self._Metric_name_bench_pearson, bench_pearson, step=step)
                tf.summary.scalar(self._Metric_name_bench_spearman, bench_spearman, step=step)

    


class CNNLSTM_3_layer_tensorboard(SpCas9_tensorboard):
    def __init__(self, HP_dict, log_dir='tensorboard/', GPU=1, random_seed=1234, verbose=1):
        super().__init__(HP_dict, log_dir=log_dir, GPU=GPU, random_seed=random_seed, verbose=verbose)
    ...
    def Model_define(self):
        # Model
        self._model_define = CNNLSTM_3_layer_SpCas9(
            HP= self._HP_dict,
            random_seed = self._random_seed
        )
        self.model = self._model_define.model
...
class CNNLSTM_regression_3_layer_tensorboard(CNNLSTM_3_layer_tensorboard):
    def __init__(self, HP_dict, log_dir='tensorboard/', GPU=1, random_seed=1234, verbose=1):
        super().__init__(HP_dict, log_dir=log_dir, GPU=GPU, random_seed=random_seed, verbose=verbose)

    def TB_HParam_define(self):
        return super().TB_HParam_define()

    def Model_define(self):
        # Model
        self._model_define = CNNLSTM_regression_3_layer_SpCas9(
            HP= self._HP_dict,
            random_seed = self._random_seed
        )
        self.model = self._model_define.model
